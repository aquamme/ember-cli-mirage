...
class Serializer {
  ...
  getHashForResource(resource, removeForeignKeys=false, didSerialize={}) {
    let hash;
    let serializer = this.serializerFor(resource.modelName);
  ...
  */
  ...
  _hashForModel(model, removeForeignKeys, didSerialize={}) {
    ...
    if (this.embed) {
      ...
      serializer.getKeysForIncluded().forEach(key => {
        let associatedResource = model[key];
        if (!_get(newDidSerialize, `${associatedResource.modelName}.${associatedResource.id}`)) {
          let [ associatedResourceHash ] = serializer.getHashForResource(associatedResource, true, newDidSerialize);
